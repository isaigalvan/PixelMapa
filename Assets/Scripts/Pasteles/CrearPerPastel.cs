...
public class CrearPerPastel : MonoBehaviour
{
    ...
    public void Crear()
    {
        GameObject personajeTemp = Instantiate(perPrefab, new Vector3(posx + 3, 0, 0), Quaternion.Euler(new Vector3(0, 0, 0)));
        personajeTemp.transform.parent = perParent;
        personajeTemp.name = "jugador1";

        GameObject personajeTemp1 = Instantiate(perPrefab, new Vector3(posx - 3, 0, 0), Quaternion.Euler(new Vector3(0, 0, 0)));
        personajeTemp1.transform.parent = perParent;
        personajeTemp1.name = "jugador2";

        per = GameObject.Find("jugador1");
        per.GetComponent<PerPastel>().numJugador = 1;

        per2 = GameObject.Find("jugador2");
        per2.GetComponent<PerPastel>().numJugador = 2;
